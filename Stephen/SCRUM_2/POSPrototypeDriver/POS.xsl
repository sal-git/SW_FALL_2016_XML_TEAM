...
<xsl:template match="/">
	...
	<html>
		<body>
			...
			<div>
				<a href="https://www.google.com"><p><b>STUDENT LEARNING OUTCOMES:</b></p></a>
				<ol>
					<b>
				...
				<ol type="A">
					...
					<li>
						...
						<ol>
							...
							<li>
								<a href="www.privacy.com" target="_blank"><b>Computer Science Electives - 15 hours (15 advanced)</b></a>
									...
									<li>
										...
										<xsl:for-each select="POS/Course">
											<ul style="list-style-type:none">
												<li>
													<xsl:if test="Divide[text()='Computer_Science_Core']">
														<a href="www.privacy.com" target="_blank"><xsl:value-of select="Prefix"/><xsl:text> </xsl:text><xsl:value-of select="Number"/><xsl:text> </xsl:text><xsl:value-of select="Name"/></a>
													</xsl:if>
												</li>
											</ul>
										</xsl:for-each>
